...
// 最坏适应算法分配内存
bool allocate_memory_worst_fit(MemoryManager* manager, int size, const char* process_name) {
    MemoryBlock* current = manager->head;
    ...
    if (worst_block != NULL) {
        ...
        if (worst_block->size == size) {
            ...
            strcpy(worst_block->process_name, process_name);
        } else {
            // 需要分割块
    ...
    while (1) {
        ...
        switch (choice) {
            case 1:
                ...
                printf("请输入需要分配的内存大小: ");
                scanf("%d", &size);
                allocate_memory_worst_fit(manager, size, process_name);
                break;
